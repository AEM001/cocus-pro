...
4）确定==少量区域==，在这个需要加强的区域附近**密集**生成正负点提示（正点表示加强/补充，负点表示去除的策略）
...
约束条件：
- 每个区域不得仅仅生成只有一种类型的点，不得生成少5个点，每个区域最多{max_total}个（正负点总和）。如果不确定，倾向于提供更多点；只有在边界看起来正确时才使用较少的点。
- 正负点必须**密集**在边界区域，正负点数量应**相近**，不要明显不平衡。正负点不能分散，必须聚焦在**区域**内。
