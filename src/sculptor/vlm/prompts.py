...
--- 先思考（私有的，不要输出） ---
1）分析当前目标的形状和边缘、纹理特征，确定在红色矩形框内哪些是目标
2）思考当前目标如何伪装在背景中，在红色矩形框内哪些是无关背景，绿色半透明覆盖层是否包含周围非主体物体
3）必须确定需要加强的==**区域**==缺失了**主体**的什么部分，以及需要去除的区域包含了**什么干扰物**
4）确定==少量区域==，在这个需要加强的区域附近生成正负点提示（正点表示加强/补充，负点表示去除的策略）
5）针对确定的少量区域中的某一个，保证精确**区分**
6）确保所有点都在红色矩形框内，且坐标有效
7）每个**区域**不得仅仅生成只有一种类型的点
8）确定好需要加强的区域并且进行**编号**i，然后确定好要该区域的**精确位置**和**精确边界**，
9）想清楚正点是要提升加强或者补充什么，负点是要去除的什么，否则不能乱生成
--- 思考结束 ---


输出要求（严格的JSON格式；不要markdown代码块）：
{{{{
  \"pos_points\": [{{{{\"point_2d\": [x, y], \"why\": \"区域编号i以及需要修改的具体**物体**\"}}}}...],
  \"neg_points\": [{{{{\"point_2d\": [x, y], \"why\": \"区域编号i以及需要去除的具体**物体**\"}}}}...]
}}}}

约束条件：
- 每个区域不得仅仅生成只有一种类型的点每个区域最多{max_total}个（正负点总和）。
- **严格限制**：所有点必须位于红色矩形框内，框外点位无效且会被忽略。
- 仅使用像素坐标（整数）：0 <= x < {image_w}, 0 <= y < {image_h}
- 保持\"why\"简短（<=15个字）说明区域编号和理由
{('额外说明：' + n) if n else ''}
"""
